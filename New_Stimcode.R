...
# Creating df's with CI for possible figures. Might scrap it. 

modmvc <- lmer(mvc ~ testdays + (1|fp) + 0, strong)

summary_df_mvc <- summary(modmvc)$coefficients %>% 
      as_tibble() %>% 
      mutate(xaxis = as.factor("testdays")) %>%
      rename(yaxis = Estimate) %>%
      mutate(ymin = yaxis - 1.96 * `Std. Error`,
             ymax = yaxis + 1.96 * `Std. Error`)

moddynmvc <- lmer(dynmvc ~ testdays + (1|fp) + 0, strong)

summary_df_dynmvc <- summary(moddynmvc)$coefficients %>% 
      as_tibble() %>% 
      mutate(xaxis = as.factor("testdays")) %>%
      rename(yaxis = Estimate) %>%
      mutate(ymin = yaxis - 1.96 * `Std. Error`,
...
MVC <- ggplot(data = subset(strong, !is.na(testday)), aes(fct_inorder(testdays), mvc, col(fp))) +
      ...
      labs(x = "", y = "MVC (Nm)", title = "Maximal Voluntary Isometric Contraction", subtitle = "A") +
      expand_limits(y=0) +
...
Dynamic <- ggplot(data = subset(strong, !is.na(dynmvc), testdays <="3"), 
      ...
      geom_boxplot(alpha = 0.5, size = .4, width = .4, fill = "grey") +
      geom_line(aes(group = fp)) +
      labs(x = "", y = "Maximal torque (Nm)", 
           title = "Maximal dynamic voluntary torque", subtitle = "B") + 
      expand_limits(y=0) +
      theme_pubr(legend = "none") +
      graph_theme
Dynamic
...
MVC_mean <- group_by(strong,testdays) %>%
      summarise(
            count = n(),
            mean.mvc = mean(mvc, na.rm = TRUE),
            sd.mvc = sd(mvc, na.rm = TRUE),
            n.mvc = n()) %>%
      mutate(se.mvc = sd.mvc / sqrt(n.mvc),
             lower.ci.mvc = mean.mvc - qt(1 - (0.05 / 2), n.mvc - 1) * se.mvc,
             upper.ci.mvc = mean.mvc + qt(1 - (0.05 / 2), n.mvc - 1) * se.mvc)
                           
MVC_mean
MVC_ttest

DescTools::MeanDiffCI(data = subset(strong, testday == "3" | testday == "4"), 
                      mvc~testday, paired = TRUE)
...
Dynamic_mean <- group_by(strong,testdays) %>%
      summarise(
            count = n(),
            mean.dynmvc = mean(dynmvc, na.rm = TRUE),
            sd.dynmvc = sd(dynmvc, na.rm = TRUE),
            n.dynmvc = n()) %>%
      mutate(se.dynmvc = sd.dynmvc / sqrt(n.dynmvc),
             lower.ci.dynmvc = mean.dynmvc - qt(1 - (0.05 / 2), n.dynmvc - 1) * se.dynmvc,
             upper.ci.dynmvc = mean.dynmvc + qt(1 - (0.05 / 2), n.dynmvc - 1) * se.dynmvc)
...
x=meanCI.data.frame(x = subset(strong, testday == "3" | testday == "4"), dynmvc~testday, paired = TRUE)
interpret(x)
# Ingen forskel mellem grupper - No group differences
...
Norm_mean <- group_by(strong, training) %>%
      summarise(
            count = n(),
            mean.fmax = mean(fmax_max*100, na.rm = TRUE),
            sd.fmax = sd(fmax_max*100, na.rm = TRUE),
            n.fmax = n()) %>%
      mutate(se.fmax = sd.fmax / sqrt(n.fmax),
             lower.ci.fmax = mean.fmax - qt(1 - (0.05 / 2), n.fmax - 1) * se.fmax,
             upper.ci.fmax = mean.fmax + qt(1 - (0.05 / 2), n.fmax - 1) * se.fmax)
Norm_mean
Norm_ttest
...
evolution_m <- ggplot(data = subset(ultra, !is.na(time)),
      ...
      labs(x = "", y = "Average muscle thickness (mm)",
           title = "Muscle thickness", subtitle = "C") +
      expand_limits(y=0)+
      graph_theme
evolution_m
...
evolution_f <- ggplot(data = subset(ultra, !is.na(time)),
      ...
      labs(x = "", y = "Average fat thickness (mm)",
           title = "Fat thickness", subtitle = "D") +
      expand_limits(y=0)+
      graph_theme
evolution_f
...
ggarrange(MVC, Dynamic, evolution_m, evolution_f, ncol = 2, nrow = 2)
ggsave("combined figure.jpg", scale = 1.5, dpi = 600)
...
Muscle_mean <- group_by(ultra,times) %>%
      summarise(
            count = n(),
            mean.wholemuscle = mean(wholemuscle, na.rm = TRUE),
            sd.wholemuscle = sd(wholemuscle, na.rm = TRUE),
            n.wholemuscle = n()) %>%
      mutate(se.wholemuscle = sd.wholemuscle / sqrt(n.wholemuscle),
             lower.ci.wholemuscle = mean.wholemuscle - qt(1 - (0.05 / 2), n.wholemuscle - 1) * se.wholemuscle,
             upper.ci.wholemuscle = mean.wholemuscle + qt(1 - (0.05 / 2), n.wholemuscle - 1) * se.wholemuscle)
Muscle_mean
Muscle_ttest

DescTools::MeanDiffCI(data = ultra, 
                      wholemuscle~time)
...
Fat_mean <- group_by(ultra,times) %>%
      summarise(
            count = n(),
            mean.wholefat = mean(wholefat, na.rm = TRUE),
            sd.wholefat = sd(wholefat, na.rm = TRUE),
            n.wholefat = n()) %>%
      mutate(se.wholefat = sd.wholefat / sqrt(n.wholefat),
             lower.ci.wholefat = mean.wholefat - qt(1 - (0.05 / 2), n.wholefat - 1) * se.wholefat,
             upper.ci.wholefat = mean.wholefat + qt(1 - (0.05 / 2), n.wholefat - 1) * se.wholefat)
Fat_mean
Fat_ttest
...
DescTools::MeanDiffCI(data = ultra_o, 
                      -wholemuscle~time, paired = TRUE)
